...
public class intake_shoot extends SubsystemBase {
    ...
    final VictorSPX rightintake = new VictorSPX(24); // 24
    ...
    final TalonFX underintake = new TalonFX(25);
    final Encoder enc = new Encoder(0, 1);//彩色線
    int mode = 0; // 0
    final Joystick controller2 = new Joystick(0);
    boolean photo = true;
    ...
    // public void intake() {
    //     photo = true;
    //     right_handSpx.set(VictorSPXControlMode.PercentOutput, 0.02 * (180 - getdis()));
    ...
    public void periodic() {
        ...
        if (controller2.getRawButton(1)) {
            ...
            double nowtime = Timer.getFPGATimestamp();
            while(nowtime - pretime < 2.0){
                rightshooter.set(VictorSPXControlMode.PercentOutput, 0.5);
                leftshooter.set(VictorSPXControlMode.PercentOutput, -0.5);
                rightintake.set(VictorSPXControlMode.PercentOutput, 0);
                leftintake.set(VictorSPXControlMode.PercentOutput, 0);
                nowtime = Timer.getFPGATimestamp();
            }
            while (nowtime - pretime > 1.5 && nowtime - pretime < 3.0) {
                rightshooter.set(VictorSPXControlMode.PercentOutput, 0.5);
                leftshooter.set(VictorSPXControlMode.PercentOutput, -0.5);
                rightintake.set(VictorSPXControlMode.PercentOutput, 0.5);
                leftintake.set(VictorSPXControlMode.PercentOutput, -0.5);
                nowtime = Timer.getFPGATimestamp();
            }
        ...
        if (photoSensor.get() == true && photo == true) {
            ...
            rightintake.set(VictorSPXControlMode.PercentOutput, 0.3);
            if (controller2.getRawButton(8)) {
                stopevery();
            }
        }
